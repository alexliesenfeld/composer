...
const WelcomePage = (props: { appStore?: AppStore, workspaceStore?: WorkspaceStore }) => {
    const addRecentlyOpenedProject = () => {
        if (props.workspaceStore!.configPath) {
            props.appStore!.addRecentlyOpenedProject(props.workspaceStore!.userConfig!.projectName,
        ...
        <div className='WelcomePage'>
            <Card>
                <div className='welcome-section'>
                    ...
                    <Button icon={NEW_OBJECT} fill={true} onClick={() => {
                        props.workspaceStore!.createNewUserConfig().then(addRecentlyOpenedProject);
                    }} intent={"primary"}>New
                        Project</Button>
                    <Button icon={DOCUMENT_OPEN} fill={true} onClick={() => {
                        ...
                        <Tree
                            contents={props.appStore!.recentlyOpenedProjects.map((metadata) => {
                                return {
                                    ...
                                    hasCaret: false,
                                    icon: DOCUMENT_OPEN,
                                    label: <Popover position={"top"}><Tooltip
                                        content={metadata.filePath}>{metadata.projectName}</Tooltip></Popover>,
                                    secondaryLabel: <div><Button small={true}
                                                                 minimal={true}
                                                                 icon={TRASH}
