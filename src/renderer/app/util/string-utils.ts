...
export const assertReplace = (fileName: string, content: string, from: string, to: string): string => {
    const {newContent, success} = replace(content, from, to);
    if (!success) {
        throw new AssertionError(`Could not replace string '${from}' to '${to}' in file ${fileName}.`)
    }
    return newContent;
};
